...
class HomePage:
    ...
    async def home_page_provide_many_items(
        ...
        query_result = self.db.query(
            Room.id, Room.number_of_beds,
            Room.price_per_day, RoomStatus.status,
            RoomPicture.picture_url
        ).join(
            RoomStatus, Room.roomStatus_idroomStatus == RoomStatus.idroomStatus
        ).join(
            RoomPicture, RoomPicture.rooms_idrooms == Room.id
        ).limit(count)
    ...
    async def home_page_provide_single_item(
        ...
        """
        query_result = self.db.query(
            Room.id, Room.description, Room.number,
            Room.number_of_beds, Room.kitchen, Room.price_per_day,
            RoomStatus.status, RoomStatus.status_description,
            RoomPicture.picture_url
        ).join(
            RoomStatus, Room.roomStatus_idroomStatus == RoomStatus.idroomStatus
        ).join(
